...
describe('Validation (US 17 & 18)', () => {
  ...
  describe('US 17 — Titre obligatoire (Manga)', () => {
  ...
  describe('US 17 — Titre obligatoire (Manhwa)', () => {
  ...
  describe('US 17 — Titre obligatoire (Manhua)', () => {
  ...
  // --- Auteur obligatoire (manga uniquement) ---

  describe('US 17 — Auteur obligatoire (Manga)', () => {
  ...
  describe('US 17 — Données valides acceptées', () => {
  ...
  // --- Pas de valeurs négatives dans UpdateChaptersDto ---

  describe('US 18 — Pas de valeurs négatives (UpdateChaptersDto)', () => {
  ...
  // --- Pas de dépassement (chapitres_lus ≤ total) ---
  // Déjà validé dans les services (US 9), ici on vérifie le contrat

  describe('US 18 — Cohérence chapitres_lus ≤ total', () => {
    it('devrait rejeter un manga avec chapitres négatifs via DTO manhwa', async () => {
      const dto = plainToInstance(AddManhwaDto, { titre: 'Test', nombreDeChapitres: -10 });
      const errors = await validate(dto);
      expect(errors.length).toBeGreaterThan(0);
    });

    it('devrait rejeter un manga avec chapitres négatifs via DTO manhua', async () => {
