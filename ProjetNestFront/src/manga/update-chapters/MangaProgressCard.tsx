...
export function MangaProgressCard({ manga, onDeactivated }: MangaProgressCardProps) {
  const [read, setRead] = useState(manga.nombreDeChapitresLus);
  const [total, setTotal] = useState(manga.nombreDeChapitres);
  const [error, setError] = useState<string | null>(null);
  const [confirming, setConfirming] = useState(false);
  const [disabling, setDisabling] = useState(false);

  const isInactive = manga.activé === false;
  ...
  async function incrementRead() {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/manga/${manga.id}/addReadChapters`, {
        method: 'PATCH',
  ...
  async function incrementTotal() {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/manga/${manga.id}/addChapters`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ value: 1 }),
      });
  ...
  async function confirmDisable() {
    ...
    setError(null);
    try {
      const res = await fetch(`${API_BASE}/disableManga/${manga.id}`, {
        method: 'PATCH',
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      setConfirming(false);
      onDeactivated?.(manga.id);
    } catch {
      setError('Une erreur est survenue, merci de réessayer.');
    } finally {
      setDisabling(false);
  ...
  return (
    <li
      className={`group relative overflow-hidden rounded-xl border border-slate-800 bg-slate-900 shadow-md shadow-black/40 ${
        isInactive ? '' : 'transition hover:border-red-500/50 hover:shadow-red-500/10'
      }`}
    >
      <div className={isInactive ? 'opacity-40 grayscale' : ''}>
        <div className="relative aspect-[2/3] overflow-hidden bg-slate-800">
          ...
          <span className="absolute left-2 top-2 rounded bg-red-600 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-widest text-white shadow">
            Manga
          </span>
        </div>

        <div className="p-4">
          <h2 className="line-clamp-2 text-base font-bold leading-tight text-white" title={manga.titre}>
            {manga.titre}
          </h2>
          <p className="mt-1 text-xs text-slate-400">par {manga.auteur}</p>
              ...
              {confirming && (
                <div className="mt-3 rounded border border-red-500/30 bg-red-950/40 p-3">
                  <p className="text-xs text-red-200">
                    Confirmer la désactivation de « {manga.titre} » ?
                  </p>
