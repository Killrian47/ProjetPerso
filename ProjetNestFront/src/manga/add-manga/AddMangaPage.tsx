...
const ENDPOINT = (import.meta.env.VITE_API_URL ?? 'http://localhost:3000') + '/addManga';

type SubmitStatus = 'idle' | 'submitting' | 'success' | 'error';

const inputClass =
  'mt-1 w-full rounded-lg border border-slate-800 bg-slate-900 px-3 py-2 text-sm text-slate-100 placeholder-slate-500 transition focus:border-red-500/60 focus:outline-none focus:ring-2 focus:ring-red-500/20';
...
export function AddMangaPage() {
  ...
  return (
    <main className="mx-auto max-w-xl p-6">
      <p className="text-xs font-bold uppercase tracking-[0.3em] text-red-500">Bibliothèque</p>
      <h1 className="mt-1 text-3xl font-black tracking-tight text-white">Ajouter un manga</h1>
      ...
      >
        ...
        <div>
          <label htmlFor="auteur" className="block text-sm font-semibold text-slate-200">
            Auteur
          </label>
          <input
            id="auteur"
        ...
        <div>
          ...
          <input
            ...
            className="mt-1 block w-full text-sm text-slate-300 file:mr-3 file:rounded file:border-0 file:bg-red-600 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-white hover:file:bg-red-500"
          />
          {previewUrl && (
            <img
              src={previewUrl}
        ...
        <button
          ...
          className="w-full rounded-lg bg-red-600 px-4 py-2.5 text-sm font-bold uppercase tracking-wider text-white transition-colors hover:bg-red-500 disabled:cursor-not-allowed disabled:opacity-50"
        >
