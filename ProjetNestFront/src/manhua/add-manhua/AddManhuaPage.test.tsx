...
import { AddManhuaPage } from './AddManhuaPage';
...
describe('AddManhuaPage — Ajouter un manhua', () => {
  beforeEach(() => {
  ...
  it('Scénario 1 — formulaire valide → submit → création réussie', async () => {
    ...
    render(<AddManhuaPage />);

    await user.type(screen.getByLabelText(/titre/i), 'Tales of Demons and Gods');
    await user.type(screen.getByLabelText(/chapitres totaux/i), '400');
    await user.click(screen.getByRole('button', { name: /ajouter/i }));

    await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(1));

    const [url, init] = fetchMock.mock.calls[0];
    expect(String(url)).toMatch(/addManhua/);
    expect(init.method).toBe('POST');
    const body = init.body as FormData;
    expect(body).toBeInstanceOf(FormData);
    expect(body.get('titre')).toBe('Tales of Demons and Gods');
    expect(body.get('nombreDeChapitres')).toBe('400');

    expect(await screen.findByText(/manhua ajouté/i)).toBeInTheDocument();
  });
  ...
  it('Scénario 4 — submit en cours → bouton désactivé', async () => {
    ...
    await user.type(screen.getByLabelText(/titre/i), 'Battle Through the Heavens');
  ...
  it('Scénario 5 — erreur backend → message d\'erreur affiché', async () => {
    ...
    await user.type(screen.getByLabelText(/titre/i), 'Soul Land');
    await user.type(screen.getByLabelText(/chapitres totaux/i), '300');
    await user.click(screen.getByRole('button', { name: /ajouter/i }));
